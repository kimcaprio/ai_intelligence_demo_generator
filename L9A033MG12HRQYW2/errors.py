...
def safe_json_parse(json_str: str, fallback: Any = None) -> Any:
    ...
    truncated responses, and extracts JSON objects or arrays from text.
    ...
    try:
        ...
        # Try direct parse on the (possibly cleaned) string first
        try:
            return json.loads(json_str.strip())
        except json.JSONDecodeError:
            # Direct parse failed, try to extract JSON intelligently
            pass
        ...
        for i, char in enumerate(json_str):
            if char == '{':
                ...
                if brace_count == 0 and start_pos >= 0:
                    ...
                    extracted_json = json_str[start_pos:i + 1]
                    try:
                        return json.loads(extracted_json)
                    except json.JSONDecodeError:
                        # This complete object is still invalid, continue searching
                        start_pos = -1
                        continue
        
        # If brace counting didn't work, try regex as fallback (greedy for complete match)
