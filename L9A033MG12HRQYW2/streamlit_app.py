    ...
    try:
        ...
        for key, table_info in structured_tables:
            ...
            if not schema:
                ...
                st.error(f"{user_msg['title']}: {key} 스키마 생성 실패")
                return None
        ...
        if len(structured_tables) > 1:
            ...
            for key, table_data in parallel_data_results.items():
                if isinstance(table_data, Exception):
                    user_msg = error_handler.get_user_friendly_message(ErrorCode.DATA_GENERATION_FAILED)
                    st.error(f"{user_msg['title']}: {key} 데이터 생성 실패: {str(table_data)[:100]}")
                    return None
        ...
        if len(unstructured_tables) == 1:
            ...
            with status_container:
                with st.expander(f"✅ {table_name} 생성됨 ({len(chunks_data)}개 청크, {lang_display})", expanded=False):
                    st.caption(f"**컬럼:** CHUNK_ID, DOCUMENT_ID, CHUNK_TEXT, DOCUMENT_TYPE, SOURCE_SYSTEM, LANGUAGE")
            ...
            for unstructured_key, result_data in unstructured_results.items():
                if isinstance(result_data, Exception):
                    st.warning(f"⚠️ {unstructured_key} 비정형 데이터 생성 실패: {str(result_data)[:100]}")
                    continue
            ...
            with ThreadPoolExecutor(max_workers=2) as executor:
                ...
                for future in as_completed(future_to_idx):
                    ...
                    try:
                        ...
                        st.warning(f"⚠️ {task_names[idx]} 생성 중 오류: {str(e)[:100]}")
        ...
        if semantic_view_info:
            with status_container:
                with st.expander(f"✅ Semantic View {semantic_view_info['view_name']} 생성됨", expanded=False):
                    # Show all tables in the view
                    table_names_str = ", ".join(semantic_view_info.get('table_names', []))
                    st.caption(f"{semantic_view_info.get('num_tables', 2)}개 테이블 결합: {table_names_str}")
                    st.caption(f"조인 키: 모든 테이블의 ENTITY_ID")
                    
                    if semantic_view_info.get('example_queries'):
                        st.caption(f"**예시 쿼리:** {len(semantic_view_info['example_queries'])}개 사전 구성됨")
                    ...
                    if grant_msgs:
                        ...
                        st.caption("⚠️ 권한 부여가 기록되지 않음 - 접근 문제가 발생할 수 있습니다")
        ...
        if search_service_name:
            with status_container:
                with st.expander(f"✅ Cortex Search Service {search_service_name} 생성됨", expanded=False):
                    st.caption(f"{unstructured_table_name}에 시맨틱 검색 활성화됨")
        ...
        if enable_agent and (semantic_view_info or search_service_name):
            ...
            if agent_config:
                ...
                if agent_config.get('error', False):
                    ...
                    with status_container:
                        with st.expander(f"✅ AI Agent {agent_config['name']} 생성됨", expanded=False):
                            st.caption(f"**위치:** {agent_config.get('schema', 'SNOWFLAKE_INTELLIGENCE.AGENTS')}")
                            st.caption(f"**표시 이름:** {agent_config.get('display_name', agent_config['name'])}")
                            st.caption(f"**도구:** {', '.join(agent_config.get('tools', []))}")
                            st.caption(f"**접속:** Snowsight → {agent_config.get('ui_path', 'AI & ML » Agents')}")
...
if st.session_state.get('show_about', False):
    ...
    with col1:
        ...
        - **Multi-Language Support** - Generate content in English, Spanish, French, German, Japanese, Chinese, and Korean
        ...
        **Step 3: Configure Infrastructure**
        ...
        - Select content language (7 languages supported)
    ...
    render_page_footer("❄️ Snowflake 기반")
    ...
    if not all_history:
        ...
        if demo_data:
            with st.container(border=True):
                ...
                if demo_data.get('industry_focus'):
                    st.info(f"🏭 **산업 분야:** {demo_data['industry_focus']}")
                
                if demo_data.get('business_value'):
                    st.info(f"💼 **비즈니스 가치:** {demo_data['business_value']}")
                    ...
                    if is_standard_mode:
                        ...
                        with col1:
                            if 'structured_1' in tables:
                                ...
                                if 'table_type' in tables['structured_1']:
                                    st.caption(f"📁 유형: {tables['structured_1']['table_type'].title()}")
                        ...
                        with col2:
                            if 'structured_2' in tables:
                                ...
                                if 'table_type' in tables['structured_2']:
                                    st.caption(f"📁 유형: {tables['structured_2']['table_type'].title()}")
                        ...
                        with col3:
                            if 'structured_3' in tables:
                                ...
                                if 'table_type' in tables['structured_3']:
                                    st.caption(f"📁 유형: {tables['structured_3']['table_type'].title()}")
                        ...
                        for i in range(3, 6):  # Check for structured_3, structured_4, structured_5
                            ...
                            if table_key in tables:
                                additional_tables.append((f"정형 테이블 {i}", table_key, tables[table_key], True))
                        
                        # Check for second unstructured table
                        if 'unstructured_2' in tables:
                            additional_tables.append(("비정형 테이블 2", "unstructured_2", tables['unstructured_2'], False))
                                ...
                                if col_idx < len(cols):  # Ensure we don't exceed column count
                                    with cols[col_idx]:
                                        ...
                                        if is_structured and 'table_type' in table_info:
                                            st.caption(f"📁 유형: {table_info['table_type'].title()}")
...
if 'demo_ideas' in st.session_state and st.session_state['demo_ideas']:
    template_label = " (템플릿 생성)" if st.session_state.get('used_fallback_demos', False) else ""
