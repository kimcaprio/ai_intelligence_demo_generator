...
def generate_tag_json(action: str, source: str, demo_name: Optional[str] = None) -> str:
    """
    Generate JSON tag string for object comments and query tags.
    
    Creates a standardized tag following the SIT tagging format for telemetry
    and object tracking.
    
    Args:
        action: Specific action being performed (e.g., "create_schema", "create_table")
        source: Source type ("sql", "python", "streamlit")
        demo_name: Optional demo name for runtime objects (schema name)
        
    Returns:
        JSON string ready for use in COMMENT or QUERY_TAG
        
    Example:
        >>> generate_tag_json("create_schema", "python", "ACME_DEMO_20250120")
        '{"origin":"sf_sit-is","name":"si-data-generator","version":{"major":2,"minor":0},"attributes":{"is_quickstart":0,"source":"python","action":"create_schema","demo_name":"ACME_DEMO_20250120"}}'
    """
    tag_data = {
        "origin": "sf_sit-is",
        "name": "si-data-generator",
        "version": {
            "major": 2,
            "minor": 0
        },
    ...
    # Add demo_name if provided (for runtime objects)
    if demo_name:
        tag_data["attributes"]["demo_name"] = demo_name
    
    # Return compact JSON (no whitespace)
    return json.dumps(tag_data, separators=(',', ':'))


def set_query_tag(session, action: str, source: str, demo_name: Optional[str] = None) -> None:
    """
    Set query tag for session to track operations in query history.
    
    Sets ALTER SESSION SET QUERY_TAG with standardized JSON format for
    ...
    Args:
        ...
        source: Source type ("sql", "python", "streamlit")
        demo_name: Optional demo name for runtime objects
        
    Example:
        >>> set_query_tag(session, "create_schema", "python", "ACME_DEMO_20250120")
    """
    try:
        tag_json = generate_tag_json(action, source, demo_name)
        # Escape single quotes in JSON for SQL string
        tag_json_escaped = tag_json.replace("'", "''")
        session.sql(f"ALTER SESSION SET QUERY_TAG = '{tag_json_escaped}'").collect()
    except Exception:
        # Silently ignore errors when setting query tag - it's optional metadata
        pass


def clear_query_tag(session) -> None:
    """
    Clear query tag from session after operations complete.
    
    Unsets the session query tag to prevent it from being applied to
    subsequent unrelated queries.
    ...
    try:
        ...
        # Silently ignore errors when clearing tag
        pass
...
def generate_demo_ideas_with_cortex(
    ...
    Args:
        ...
        target_question_analysis: Optional analysis dict from analyze_target_questions() (Phase 0)
    ...
    # Use prompt from prompts module with Phase 0 enhancements
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_key):
            ...
            try:
                ...
                results[key] = e
                st.warning(f"⚠️ Error in parallel task '{key}': {type(e).__name__}: {str(e)[:100]}")
                with st.expander(f"🔍 Error details for {key}"):
...
SUPPORTED_LANGUAGES = {
    ...
    "ko": {
        ...
        "native_name": "한국어",
        "cortex_instruction": (
            "Generate content in Korean (한국어). Use proper Korean "
            "grammar, vocabulary, and natural Korean expressions with "
            "appropriate honorifics and formal/informal speech levels."
        ),
        "encoding": "utf-8",
        "sample_validation": r"[\uAC00-\uD7AF\u1100-\u11FF\u3130-\u318F]+",
        "char_validation": (
            r"[\uAC00-\uD7AF\u1100-\u11FF\u3130-\u318F\s\d\.,;:!?"
            r"（）「」『』【】、。·~]+"
        ),
