...
from utils import LLM_MODEL, generate_tag_json, set_query_tag, clear_query_tag
...
def generate_tool_resources(
    ...
    # ============ Custom Tool Resources ============
    # Note: 이 함수는 현재 사용되지 않음 - generate_agent_json_spec()에서 inline으로 정의됨
    # 참고용으로만 유지
    
    # Naver News Search Function - caller_warehouse 사용 (사용자의 기본 warehouse)
    tool_resources["naver_news_search"] = {
        "execution_environment": {"type": "caller_warehouse"},
        "identifier": "DEMO_DB.APPLICATIONS.NAVER_NEWS_SEARCH",
        "name": "NAVER_NEWS_SEARCH(VARCHAR, VARCHAR, VARCHAR)",
        "type": "function"
    }
    
    # # Naver Shopping Price Compare Procedure
    # tool_resources["naver_shopping_price_compare"] = {
    #     "execution_environment": {"type": "caller_warehouse"},
    #     "identifier": "DEMO_DB.APPLICATIONS.NAVER_SHOPPING_COMPARE_PRICE",
    #     "name": "NAVER_SHOPPING_COMPARE_PRICE(VARCHAR, DEFAULT VARCHAR, DEFAULT VARCHAR, DEFAULT NUMBER)",
    #     "type": "procedure"
    # }
    
    # Naver Shopping Trends Procedure
    tool_resources["naver_shopping_trends"] = {
        "execution_environment": {"type": "caller_warehouse"},
        "identifier": "DEMO_DB.APPLICATIONS.NAVER_SHOPPING_TRENDS",
        "name": "NAVER_SHOPPING_TRENDS(VARCHAR, DEFAULT NUMBER, DEFAULT VARCHAR, DEFAULT VARCHAR, DEFAULT NUMBER)",
        "type": "procedure"
    }
...
def generate_agent_json_spec(
    ...
    # Build fully qualified names for resources
    # Ensure we have DATABASE.SCHEMA format for resource names
    if '.' in schema_name:
        full_schema_name = schema_name
    elif session:
        try:
            database_name = session.get_current_database()
            full_schema_name = f"{database_name}.{schema_name}" if database_name else schema_name
        except:
            full_schema_name = schema_name
    else:
        full_schema_name = schema_name
    
    # Build fully qualified resource names from function parameters (NOT demo_data)
    analyst_semantic_view_fqn = f"{full_schema_name}.{semantic_view_name}_SEMANTIC_MODEL" if semantic_view_name else None
    search_service_fqn = f"{full_schema_name}.{search_service_name}" if search_service_name else None
    
    tools = []
    
    # === Built-in tools ===
    # Cortex Analyst - 함수 파라미터 semantic_view_name 사용
    if has_analyst:
        tools.append({
            "tool_spec": {
                "type": "cortex_analyst_text_to_sql",
                "name": "Query Demo Data",
                "description": f"Query structured data tables for {demo_data.get('title', 'demo')}. Use this tool to analyze data, find trends, calculate metrics, and answer questions about the structured tables."
            }
        })
    
    # Cortex Search - 함수 파라미터 search_service_name 사용
    if has_search:
        tools.append({
            "tool_spec": {
                "type": "cortex_search",
                "name": "Search Documents",
                "description": f"Search unstructured documents for {demo_data.get('title', 'demo')}. Use this tool to find relevant information in documents, policies, research, and text content."
            }
        })
    
    # === Custom tools ===
    # Naver News Search - 뉴스 검색 API
    tools.append({
        "tool_spec": {
            "type": "generic",
            "name": "naver_news_search",
            "description": "PROCEDURE/FUNCTION DETAILS:\n- Type: Custom Function\n- Language: Python\n- Signature: (QUERY VARCHAR, START_DATE VARCHAR, END_DATE VARCHAR)\n- Returns: VARIANT (JSON object)\n\nDESCRIPTION:\nSearch Naver news articles by keyword with optional date filters. Use this to find recent news, press releases, and media coverage about companies, products, or topics.\n\nUSAGE SCENARIOS:\n- Find recent news articles about a company or product\n- Research media coverage for competitive analysis\n- Monitor industry trends and announcements",
            "input_schema": {
                "type": "object",
                "properties": {
                    "query": {"type": "string", "description": "Search keyword for news articles"},
    ...
    # # Naver Shopping Price Compare - 가격 비교 API
    # tools.append({
    #     "tool_spec": {
    #         "type": "generic",
    #         "name": "naver_shopping_price_compare",
    #         "description": "PROCEDURE/FUNCTION DETAILS:\n- Type: Custom Procedure\n- Language: Python\n- Signature: (KEYWORD VARCHAR, PRODUCT_ID VARCHAR, CATEGORY_FILTER VARCHAR, TOP_N NUMBER)\n- Returns: VARIANT (JSON object)\n\nDESCRIPTION:\nCompare product prices across different sellers on Naver Shopping. Use this to find the lowest price, analyze price distribution, and compare sellers for a specific product.\n\nUSAGE SCENARIOS:\n- Compare prices for a product across multiple sellers\n- Find the best deal for a specific item\n- Analyze price distribution and seller competition",
    #         "input_schema": {
    #             "type": "object",
    #             "properties": {
    #                 "keyword": {"type": "string", "description": "Product name or search keyword"},
    #                 "product_id": {"type": "string", "description": "Optional specific product ID"},
    #                 "category_filter": {"type": "string", "description": "Category path like '패션의류>남성의류'"},
    #                 "top_n": {"type": "number", "description": "Number of results to return (default 100)"}
    #             },
    #             "required": ["keyword"]
    #         }
    #     }
    ...
    # Naver Shopping Trends - 쇼핑 트렌드 분석 API
    tools.append({
        "tool_spec": {
            "type": "generic",
            "name": "naver_shopping_trends",
            "description": "PROCEDURE/FUNCTION DETAILS:\n- Type: Custom Procedure\n- Language: Python\n- Signature: (KEYWORD VARCHAR, MIN_PRICE NUMBER, START_DATE VARCHAR, END_DATE VARCHAR, TOP_N NUMBER)\n- Returns: VARIANT (JSON object)\n- Execution: OWNER with CALLED ON NULL INPUT\n- Volatility: VOLATILE\n- Primary Function: Naver Shopping API integration for product price analysis and demographic trend insights\n\nDESCRIPTION:\nThis function integrates with Naver Shopping APIs to provide comprehensive market intelligence by analyzing product pricing and demographic trends for specified keywords. The function performs dual analysis: it searches for products above a minimum price threshold and retrieves age-based shopping trend data over a specified time period.\n\nUSAGE SCENARIOS:\n- Market research teams analyzing competitor pricing and identifying optimal price points\n- E-commerce businesses conducting competitive analysis to adjust pricing strategies\n- Marketing departments researching demographic trends and age-based shopping patterns",
            "input_schema": {
                "type": "object",
                "properties": {
                    "keyword": {"type": "string", "description": "Product name or search keyword"},
                    "min_price": {"type": "number", "description": "Minimum price filter (default 5000)"},
                    "start_date": {"type": "string", "description": "Start date in YYYY-MM-DD format"},
                    "end_date": {"type": "string", "description": "End date in YYYY-MM-DD format for trend analysis"},
                    "top_n": {"type": "number", "description": "Number of results to return (default 10)"}
                },
                "required": ["keyword", "min_price", "start_date", "end_date", "top_n"]
            }
        }
    })
    
    # === Tool resources ===
    tool_names = {t["tool_spec"]["name"] for t in tools}
    tool_resources = {}
    
    # 1) Cortex Analyst - 함수 파라미터로 전달된 semantic_view_name 사용
    if "Query Demo Data" in tool_names and analyst_semantic_view_fqn:
        tool_resources["Query Demo Data"] = {
            "semantic_view": analyst_semantic_view_fqn,
            "execution_environment": {"type": "caller_warehouse"}
        }
    
    # 2) Cortex Search - 함수 파라미터로 전달된 search_service_name 사용
    if "Search Documents" in tool_names and search_service_fqn:
        tool_resources["Search Documents"] = {
            "name": search_service_fqn,
            "max_results": 5,
            "id_column": "CHUNK_ID",
            "execution_environment": {"type": "caller_warehouse"}
        }
    
    # 3) Custom tools - caller_warehouse 사용 (사용자의 기본 warehouse)
    if "naver_news_search" in tool_names:
        tool_resources["naver_news_search"] = {
            "execution_environment": {"type": "caller_warehouse"},
            "identifier": "DEMO_DB.APPLICATIONS.NAVER_NEWS_SEARCH",
            "name": "NAVER_NEWS_SEARCH(VARCHAR, VARCHAR, VARCHAR)",
    ...
    # if "naver_shopping_price_compare" in tool_names:
    #     tool_resources["naver_shopping_price_compare"] = {
    #         "execution_environment": {"type": "caller_warehouse"},
    #         "identifier": "DEMO_DB.APPLICATIONS.NAVER_SHOPPING_COMPARE_PRICE",
    #         "name": "NAVER_SHOPPING_COMPARE_PRICE(VARCHAR, DEFAULT VARCHAR, DEFAULT VARCHAR, DEFAULT NUMBER)",
    ...
    if "naver_shopping_trends" in tool_names:
        tool_resources["naver_shopping_trends"] = {
            "execution_environment": {"type": "caller_warehouse"},
            "identifier": "DEMO_DB.APPLICATIONS.NAVER_SHOPPING_TRENDS",
            "name": "NAVER_SHOPPING_TRENDS(VARCHAR, DEFAULT NUMBER, DEFAULT VARCHAR, DEFAULT VARCHAR, DEFAULT NUMBER)",
    ...
    # === 일관성 검증: 툴 ↔ 리소스 키 완전 일치 보장 ===
    # Note: generate_tool_resources()는 더 이상 호출하지 않음 - 위에서 inline으로 정의됨
    assert tool_names == set(tool_resources.keys()), \
        f"Mismatch: tools={tool_names} vs resources={set(tool_resources.keys())}"
    ...
    # NOTE: Only orchestration model/instructions are valid for agent spec
    spec = {
        "models": {
...
def create_agent_automatically(
    ...
    try:
        ...
        tools_config.extend([
            'Naver News Search',
            'Naver Shopping Price Compare',
            'Naver Shopping Trends'
        ])
        ...
        # Use dollar-quoting for PROFILE to handle special characters in display_name
        profile_json = json.dumps({"display_name": display_name})
        
        # Generate comment tag for agent
        comment_tag = generate_tag_json("create_agent", "python", demo_name=schema_name)
        comment_escaped = comment_tag.replace("'", "''")
...
"""
        ...
        # Set query tag for agent creation
        set_query_tag(session, "create_agent", "python", demo_name=schema_name)
...
def create_cortex_search_service(
    ...
    try:
        ...
        # Set query tag for search service creation
        set_query_tag(session, "create_cortex_search_service", "python", demo_name=schema_name)
        
        # Create search service
        session.sql(create_service_sql).collect()
        
        # Add comment tag to search service
        comment_tag = generate_tag_json("create_cortex_search_service", "python", demo_name=schema_name)
        try:
            session.sql(f"ALTER CORTEX SEARCH SERVICE {full_schema_name}.{service_name} SET COMMENT = '{comment_tag}'").collect()
        except Exception:
            # Some Snowflake versions may not support COMMENT on search services - continue anyway
            pass
...
def get_synonym_generation_prompt(column_name: str, column_type: str, table_name: str) -> str:
    """
    Generate prompt for LLM to create intelligent synonyms for a column.
    
    Args:
        column_name: Name of the column (e.g., 'GEOGRAPHIC_SCOPE')
        column_type: Data type (e.g., 'VARCHAR')
        table_name: Table containing the column
        
    Returns:
        Formatted prompt string for LLM
    """
    return f"""Given this database column, generate 3-5 natural language synonyms that a business user might use to refer to this data.

Column: {column_name}
Type: {column_type}
Table: {table_name}

Generate synonyms that capture:
1. Natural language equivalents (e.g., GEOGRAPHIC_SCOPE → "geographic scope", "geography", "location scope")
2. Common business terms (e.g., BUDGET_AMOUNT → "budget", "campaign budget", "marketing spend")
3. Plural/singular variations (e.g., "customer", "customers")
4. Domain-specific terminology relevant to the column name

Return ONLY a JSON array of 3-5 strings, nothing else:
["synonym1", "synonym2", "synonym3"]

Examples:
- GEOGRAPHIC_SCOPE → ["geographic scope", "geography", "location scope", "regional scope", "area"]
- BUDGET_AMOUNT → ["budget", "campaign budget", "marketing spend", "investment", "cost"]
- TOTAL_LIFETIME_VALUE → ["lifetime value", "CLV", "customer value", "total value", "LTV"]
- SATISFACTION_RATING → ["satisfaction", "rating", "customer satisfaction", "satisfaction score"]
- RESTAURANT_ID → ["restaurant", "location", "store", "restaurant ID", "location ID"]
- EMPLOYEE_NAME → ["employee", "staff", "worker", "team member", "personnel"]

Be specific and relevant to the column name. Avoid generic terms unless appropriate."""


def add_intelligent_calculated_facts(
    tables_info: List[Dict],
    all_tables_columns: Dict,
    facts_list: List[str],
    used_aliases: set,
    first_table_name: str
) -> List[str]:
    """
    Intelligently add calculated metric fields based on available columns.
    
    This function detects common patterns and adds calculated metrics to enable
    the agent to answer questions about percentages, averages, ratios, growth rates, etc.
    
    Patterns detected:
    - AMOUNT + QUANTITY → AVG_PRICE, AMOUNT_PER_UNIT
    - Multiple amount columns → TOTAL (sum of amounts)
    - CONVERSIONS + VISITS → CONVERSION_RATE
    - Any amount + CATEGORY → Enables percentage calculations
    - DATE + AMOUNT → Enables growth rate calculations
    - SCORE columns → MIN, MAX, AVG aggregates
    
    Args:
        tables_info: List of table information dictionaries
        all_tables_columns: Dictionary mapping table names to column lists
        facts_list: Existing facts list (for reference, not modified)
        used_aliases: Set of already-used column aliases
        first_table_name: Name of the first table (for fallback)
        
    Returns:
        List of calculated fact SQL strings to add to semantic view
    """
    from utils import MAX_TOTAL_FACTS
    
    calculated_facts = []
    
    # Track column patterns across all tables
    amount_columns = []  # (table, column_name, type)
    quantity_columns = []
    date_columns = []
    category_columns = []
    score_columns = []
    ...
    # First pass: identify column patterns
    for table_info in tables_info:
        ...
        for col in table_columns:
            ...
            # Identify numeric columns by semantic meaning
            if any(num_type in col_type for num_type in ['NUMBER', 'INT', 'FLOAT', 'DECIMAL', 'DOUBLE']):
                # Amount/value columns
                if any(keyword in col_name for keyword in ['AMOUNT', 'VALUE', 'REVENUE', 'SALES', 'COST', 'PRICE', 'TOTAL']):
                    amount_columns.append((table_name, col['name'], col_type))
                # Quantity/count columns
                elif any(keyword in col_name for keyword in ['QUANTITY', 'QTY', 'COUNT', 'UNITS', 'VOLUME']):
                    quantity_columns.append((table_name, col['name'], col_type))
                # Score/rating columns
                elif any(keyword in col_name for keyword in ['SCORE', 'RATING', 'RANK', 'GRADE', 'PERFORMANCE']):
                    score_columns.append((table_name, col['name'], col_type))
                # Conversion/engagement metrics
                elif any(keyword in col_name for keyword in ['CONVERSION', 'VISIT', 'CLICK', 'VIEW', 'ENGAGEMENT']):
                    count_columns.append((table_name, col['name'], col_type))
            
            # Identify date columns
            if any(date_type in col_type for date_type in ['DATE', 'TIMESTAMP', 'TIME']):
                date_columns.append((table_name, col['name'], col_type))
            
            # Identify category/grouping columns
            if any(text_type in col_type for text_type in ['VARCHAR', 'STRING', 'TEXT']):
                if any(keyword in col_name for keyword in ['CATEGORY', 'TYPE', 'STATUS', 'SEGMENT', 'GROUP', 'CLASS']):
                    category_columns.append((table_name, col['name'], col_type))
    
    # Second pass: generate calculated metrics based on detected patterns
    
    # Pattern 1: AMOUNT / QUANTITY → AVG_PRICE or AMOUNT_PER_UNIT
    if amount_columns and quantity_columns and len(calculated_facts) < MAX_TOTAL_FACTS - len(facts_list):
        # Pick the most promising pair (prefer same table)
        for amt_table, amt_col, amt_type in amount_columns[:2]:  # Limit to first 2 amounts
            for qty_table, qty_col, qty_type in quantity_columns[:2]:  # Limit to first 2 quantities
                if len(calculated_facts) >= 3:  # Don't add too many calculated metrics
                    break
                
                # Generate alias name
                if 'PRICE' in amt_col.upper() or 'AMOUNT' in amt_col.upper():
                    calc_alias = f"AVG_{amt_col}_PER_{qty_col}"
                else:
                    calc_alias = f"{amt_col}_PER_{qty_col}"
                
                # Avoid duplicates
                if calc_alias not in used_aliases and len(calc_alias) < 50:
                    calc_sql = f"    ({amt_table}.{amt_col} / NULLIF({qty_table}.{qty_col}, 0)) as {calc_alias} with synonyms=('average','per unit','unit price','price per','ratio') comment='Calculated: {amt_col} divided by {qty_col}'"
                    calculated_facts.append(calc_sql)
                    used_aliases.add(calc_alias)
    
    # Pattern 2: CONVERSIONS / VISITS → CONVERSION_RATE (percentage)
    conversion_col = next((col for col in count_columns if 'CONVERSION' in col[1].upper()), None)
    visit_col = next((col for col in count_columns if any(kw in col[1].upper() for kw in ['VISIT', 'SESSION', 'VIEW'])), None)
    
    if conversion_col and visit_col and len(calculated_facts) < MAX_TOTAL_FACTS - len(facts_list):
        calc_alias = "CONVERSION_RATE_PCT"
        if calc_alias not in used_aliases:
            calc_sql = f"    (({conversion_col[0]}.{conversion_col[1]} / NULLIF({visit_col[0]}.{visit_col[1]}, 0)) * 100) as {calc_alias} with synonyms=('conversion rate','conversion percentage','success rate') comment='Calculated: Conversion rate as percentage'"
            calculated_facts.append(calc_sql)
            used_aliases.add(calc_alias)
    
    # Pattern 3: Multiple amount columns → Add a TOTAL metric
    if len(amount_columns) >= 2 and len(calculated_facts) < MAX_TOTAL_FACTS - len(facts_list):
        # Only if they're from the same table (can be summed)
        same_table_amounts = {}
        for table, col, col_type in amount_columns:
            if table not in same_table_amounts:
                same_table_amounts[table] = []
            same_table_amounts[table].append(col)
        
        for table, cols in same_table_amounts.items():
            if len(cols) >= 2:
                calc_alias = "TOTAL_AMOUNT"
                if calc_alias not in used_aliases:
                    sum_expr = " + ".join([f"{table}.{col}" for col in cols[:3]])  # Max 3 columns
                    calc_sql = f"    ({sum_expr}) as {calc_alias} with synonyms=('total','sum','combined','aggregate') comment='Calculated: Sum of multiple amount columns'"
                    calculated_facts.append(calc_sql)
                    used_aliases.add(calc_alias)
                break  # Only add one total
    
    # Pattern 4: Score columns → Add AVG_SCORE if multiple score columns exist
    if len(score_columns) >= 2 and len(calculated_facts) < MAX_TOTAL_FACTS - len(facts_list):
        same_table_scores = {}
        for table, col, col_type in score_columns:
            if table not in same_table_scores:
                same_table_scores[table] = []
            same_table_scores[table].append(col)
        
        for table, cols in same_table_scores.items():
            if len(cols) >= 2:
                calc_alias = "AVG_SCORE"
                if calc_alias not in used_aliases:
                    avg_expr = " + ".join([f"{table}.{col}" for col in cols[:3]])
                    avg_expr = f"({avg_expr}) / {min(len(cols), 3)}"
                    calc_sql = f"    {avg_expr} as {calc_alias} with synonyms=('average score','mean score','overall score','combined score') comment='Calculated: Average of multiple score columns'"
                    calculated_facts.append(calc_sql)
                    used_aliases.add(calc_alias)
                break
    
    # Limit total calculated facts to avoid overwhelming the semantic view
    max_calculated = 5
    if len(calculated_facts) > max_calculated:
        calculated_facts = calculated_facts[:max_calculated]
    
    return calculated_facts


@timeit
def batch_generate_synonyms(all_tables_columns: Dict, session, error_handler) -> Dict[str, str]:
    """
    Batch generate synonyms for all columns in parallel using LLM.
    
    This significantly improves performance by generating all synonyms concurrently
    instead of serially for each column.
    
    Args:
        all_tables_columns: Dict mapping table_name -> list of column metadata
        session: Snowflake session for LLM calls
        error_handler: Error handler instance
        
    Returns:
        Dict mapping 'table_name.column_name' -> synonym string (e.g., "'budget', 'cost', 'spend'")
    """
    try:
        from utils import execute_parallel_llm_calls, call_cortex_with_retry
        
        # Handle empty input
        if not all_tables_columns:
            return {}
        
        # Build dictionary of tasks for parallel execution
        # Format: {task_key: (function, args)}
        synonym_tasks = {}
        for table_name, columns in all_tables_columns.items():
            if not columns:
                continue
            for col in columns:
                # Skip ENTITY_ID as it gets special treatment
                if col.get('name') != 'ENTITY_ID':
                    task_key = f"{table_name}.{col['name']}"
                    prompt = get_synonym_generation_prompt(col['name'], col.get('type', 'VARCHAR'), table_name)
                    # Each task is (function, [args])
                    synonym_tasks[task_key] = (call_cortex_with_retry, [prompt, session, error_handler])
        
        # If no tasks, return empty dict
        if not synonym_tasks:
            return {}
        
        # Execute in parallel (max 10 at a time to avoid rate limits)
        synonym_results = execute_parallel_llm_calls(
            tasks=synonym_tasks,
            max_workers=10
        )
        
        # Parse results into column_synonym_map
        column_synonym_map = {}
        for task_key, result in synonym_results.items():
            try:
                import json
                if not result:
                    column_synonym_map[task_key] = "'value', 'data', 'field'"
                    continue
                ...
                # Return as comma-separated quoted string for SQL
                synonym_str = ", ".join([f"'{str(s)}'" for s in synonyms[:5] if s])  # Limit to 5
                column_synonym_map[task_key] = synonym_str if synonym_str else "'value', 'data', 'field'"
            except Exception as e:
                # Fallback to basic synonyms if parsing fails
                column_synonym_map[task_key] = "'value', 'data', 'field'"
        
        # Initialize synonym generation metric (was_needed determined later by question validation)
        import streamlit as st
        if 'schema_validation_metrics' not in st.session_state:
            st.session_state['schema_validation_metrics'] = {}
        
        # Note: The 'time' will be set by @timeit decorator in metrics
        # We're just initializing the tracking structure here
        if 'synonym_generation' not in st.session_state['schema_validation_metrics']:
            st.session_state['schema_validation_metrics']['synonym_generation'] = {
                'was_needed': False,  # Updated by question validation
                'synonyms_generated': len(column_synonym_map),
                'natural_language_questions': 0,
                'issues_caught': "Not yet determined"
            }
        
        return column_synonym_map
    except Exception as e:
        # If entire function fails, return empty dict to not block infrastructure creation
        import streamlit as st
        st.warning(f"⚠️ Synonym generation failed, using generic synonyms: {str(e)}")
        return {}
...
def create_semantic_view(
    ...
    # Sanitize company_name: replace '-' and spaces with '_' for valid SQL identifier
    sanitized_company_name = company_name.replace('-', '_').replace(' ', '_').upper()
    view_name = f"{sanitized_company_name}_SEMANTIC_VIEW_SEMANTIC_MODEL"
    ...
    # Batch generate intelligent synonyms for all columns in parallel
    column_synonym_map = batch_generate_synonyms(all_tables_columns, session, error_handler)
    ...
    for table_info in tables_info:
        ...
        for col in table_columns:
            if col['name'] != 'ENTITY_ID' and any(num_type in col['type'].upper() for num_type in ['NUMBER', 'INT', 'FLOAT', 'DECIMAL', 'DOUBLE']):
                ...
                if col['name'] not in used_aliases:
                    # Get intelligent synonyms for this column
                    col_key = f"{table_name}.{col['name']}"
                    synonyms = column_synonym_map.get(col_key, "'value', 'amount', 'quantity'")
                    facts_list.append(f"    {table_name}.{col['name']} as {col['name']} with synonyms=({synonyms}) comment='Numeric value from {table_name}'")
        ...
        for col in table_columns:
            if col['name'] != 'ENTITY_ID' and col['name'] not in used_aliases:
                ...
                # Get intelligent synonyms for this column
                col_key = f"{table_name}.{col['name']}"
                synonyms = column_synonym_map.get(col_key, "'category', 'type', 'label'")
                
                # Check date/time types first
                if any(date_type in col_type_upper for date_type in ['TIMESTAMP', 'DATE']) or col_type_upper == 'TIME':
                    dimensions_list.append(f"    {table_name}.{col['name']} as {col['name']} with synonyms=({synonyms}) comment='Date/time dimension from {table_name}'")
                    used_aliases.add(col['name'])
                elif 'BOOLEAN' in col_type_upper:
                    dimensions_list.append(f"    {table_name}.{col['name']} as {col['name']} with synonyms=({synonyms}) comment='Boolean dimension from {table_name}'")
                    used_aliases.add(col['name'])
                elif any(text_type in col_type_upper for text_type in ['VARCHAR', 'STRING', 'TEXT', 'CHAR']):
                    dimensions_list.append(f"    {table_name}.{col['name']} as {col['name']} with synonyms=({synonyms}) comment='Text dimension from {table_name}'")
                    used_aliases.add(col['name'])
                
                if len(dimensions_list) >= MAX_TOTAL_DIMENSIONS:
                    break
    
    # NEW: Add intelligent calculated metrics based on available columns
    # This enables the agent to answer questions about percentages, averages, ratios, etc.
    calculated_facts = add_intelligent_calculated_facts(
        tables_info=tables_info,
        all_tables_columns=all_tables_columns,
        facts_list=facts_list.copy(),
        used_aliases=used_aliases.copy(),
        first_table_name=first_table_name
    )
    facts_list.extend(calculated_facts)
    ...
    # Generate comment tag for semantic view
    comment_tag = generate_tag_json("create_semantic_view", "python", demo_name=schema_name)
...
COMMENT = '{comment_tag}'"""
    ...
    try:
        # Set query tag for semantic view creation
        set_query_tag(session, "create_semantic_view", "python", demo_name=schema_name)
        ...
        return {
            ...
            "column_synonyms": column_synonym_map  # Include intelligent synonyms for validation
        }
    except Exception as e:
        error_handler.log_error(
        ...
        return {
            ...
            "create_sql": semantic_sql,
            "column_synonyms": column_synonym_map if 'column_synonym_map' in locals() else {}
        }
