...
def timeit(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        if 'metrics_self' not in st.session_state:
            st.session_state['metrics_self'] = {}
        if 'metrics_call_stack' not in st.session_state:
            st.session_state['metrics_call_stack'] = []
        
        # Track call stack depth (thread-safe: only append if not in parallel context)
        # In parallel execution, skip call stack tracking to avoid race conditions
        import threading
        is_main_thread = threading.current_thread() == threading.main_thread()
        
        if is_main_thread:
            st.session_state['metrics_call_stack'].append(func.__name__)
        
        start_time = time.time()
        child_time_before = sum(st.session_state['metrics_self'].values())
        ...
        try:
            ...
            child_time_after = sum(st.session_state['metrics_self'].values())
            
            # Calculate self time (excluding time spent in nested @timeit functions)
            child_time_in_this_call = child_time_after - child_time_before
            self_time = elapsed_time - child_time_in_this_call
            
            # Accumulate times for functions called multiple times
            if func.__name__ in st.session_state['metrics']:
                st.session_state['metrics'][func.__name__] += elapsed_time
                st.session_state['metrics_self'][func.__name__] += self_time
            else:
                st.session_state['metrics'][func.__name__] = elapsed_time
                st.session_state['metrics_self'][func.__name__] = self_time
            
            # Pop from call stack (only if we pushed)
            if is_main_thread and st.session_state['metrics_call_stack']:
                try:
                    st.session_state['metrics_call_stack'].pop()
                except IndexError:
                    pass  # Ignore if list is empty (shouldn't happen, but be safe)
...
def display_performance_summary():
    ...
    if 'metrics' in st.session_state and st.session_state['metrics']:
        ...
        st.caption("'Self Time' = time in function excluding nested calls (more accurate)")
        
        # Get self-time metrics
        self_metrics = st.session_state.get('metrics_self', {})
        
        for func_name, total_duration in sorted_metrics:
            self_duration = self_metrics.get(func_name, total_duration)
            percentage = (self_duration / total_time * 100) if total_time > 0 else 0
            elapsed_time = self_duration
            st.caption(f"**{func_name}**: {elapsed_time:.2f}s ({percentage:.1f}%)")
            
            # Show nested time if significant
            nested_time = total_duration - self_duration
            if nested_time > 0.5:
                st.caption(f"   ↳ includes {nested_time:.2f}s in nested calls")
